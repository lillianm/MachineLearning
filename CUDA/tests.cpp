      ...
      for (int i = 0; i < TestFrameWork::size; i++)
        {
          ...
          if(TestFrameWork::matrix_dim[i] == 17){
          for(int k =0;k<TestFrameWork::matrix_dim[i];k++){
            for(int j = 0; j< TestFrameWork::matrix_dim[i];j++){
              printf("%f | \t", result[k * TestFrameWork::matrix_dim[i] + j]);
            }
            printf("\n");
          }
        }
          ...
          if(TestFrameWork::matrix_dim[i] == 17){
            for(int k =0;k<TestFrameWork::matrix_dim[i];k++){
              printf("line: %d\n",k );
            for(int j = 0; j< TestFrameWork::matrix_dim[i];j++){
              //if( (answers[k * TestFrameWork::matrix_dim[i] + j] - result[k * TestFrameWork::matrix_dim[i] + j]) != 0){
                //  printf("%d:%d",k,j);
              //}

              printf("%.1f |  ", (answers[k * TestFrameWork::matrix_dim[i] + j] - result[k * TestFrameWork::matrix_dim[i] + j]));
            }
            printf("\n");
          }
        }
